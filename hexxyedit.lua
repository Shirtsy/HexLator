...
-- Create .hexpat files by default
if not fs.exists(sPath) and not string.find(sPath, "%.") then
    local sExtension = "hexpat" --settings.get("edit.default_extension")
...
-- Where all the text and stuff is stored
local tLines = {}
local bRunning = true

-- Colours
local highlightColour, keywordColour, keyword2Colour, commentColour, textColour, bgColour, stringColour
if term.isColour() then
    bgColour = colours.black
    textColour = colours.white
    highlightColour = colours.yellow
    keywordColour = colours.yellow
    keyword2Colour = colours.purple
    ...
    while #sLine > 0 do
        sLine =
            ...
            tryWrite(sLine, "^[%w_]+", function(match)
                ...
                elseif tKeywords2[match] then
                    return keyword2Colour
                end
...
--Testing out my own autocomplete
--local tCompleteEnv = _ENV
local tCompleteEnv = symbolCompletions
...
--This is where I would add my own menu functions
local tMenuFuncs = {
    Save = function()
        if bReadOnly then
